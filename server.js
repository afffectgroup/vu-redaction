...
// ── Routes pages (Option B — multi-pages EJS) ─────────────────
const PAGES = {
  'dashboard':    'Dashboard',
  'veille':       'Veille',
...
Object.entries(PAGES).forEach(([slug, title]) => {
  app.get(`/${slug}`, (req, res) => {
    // Si la vue EJS existe → la rendre ; sinon fallback sur index.html (migration progressive)
    const viewPath = path.join(__dirname, 'views', `${slug}.ejs`);
    const fs = require('fs');
    if (fs.existsSync(viewPath)) {
      res.render(slug, { pageTitle: title, query: req.query });
    } else {
      // Fallback : SPA pendant la migration
